...
import { isValidHardenedPath } from '@mysten/sui/cryptography';
import {
  Ed25519Keypair,
  Ed25519PublicKey,
...
export default class SUIWallet extends CsWallet {
  ...
  static tokenUrl(platform, address, development) {
    if (development) {
      return `https://suiscan.xyz/testnet/coin/${address}`;
    }
    return `https://suiscan.xyz/mainnet/coin/${address}`;
  }
  ...
  validateDerivationPath(path) {
    return /^m(\/\d+')*$/.test(path) && isValidHardenedPath(path);
  }
