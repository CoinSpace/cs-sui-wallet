...
export default class SUIWallet extends CsWallet {
  ...
  async create(seed) {
    ...
    this.#publicKey = Ed25519Keypair.deriveKeypairFromSeed(seed, this.settings.bip44).getPublicKey();
  ...
  getPrivateKey(seed) {
    ...
    return [{
      ...
      privatekey: Ed25519Keypair.deriveKeypairFromSeed(seed, this.settings.bip44).getSecretKey(),
    }];
  }
  ...
  async createTransaction({ address, amount, price }, seed) {
    ...
    const { value } = amount;
    const keypair = Ed25519Keypair.deriveKeypairFromSeed(seed, this.settings.bip44);
    if (this.crypto.type === 'coin') {
      const csFeeConfig = await this.getCsFeeConfig();
      const csFeeValue = await this.calculateCsFee({ value, price });
      const minerFee = await this.#estimateMinerFee({
