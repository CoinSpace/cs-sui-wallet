...
export default class SUIWallet extends CsWallet {
  ...
  async create(seed) {
    ...
    this.#publicKey = Ed25519Keypair.deriveKeypairFromSeed(toHex(seed)).getPublicKey();
  ...
  async load() {
    ...
    try {
      this.#coinObjects = await this.#api.getObjects();
      this.#coinBalance = this.#coinObjects.reduce((sum, obj) => sum + BigInt(obj.balance), 0n);
      if (this.crypto.type === 'coin') {
        this.storage.set('balance', this.#coinBalance.toString());
      }
      if (this.crypto.type === 'token') {
        this.#tokenObjects = await this.#api.getObjects(this.crypto.address);
        this.#tokenBalance = this.#tokenObjects.reduce((sum, obj) => sum + BigInt(obj.balance), 0n);
  ...
  getPrivateKey(seed) {
    ...
    return [{
      ...
      privatekey: Ed25519Keypair.deriveKeypairFromSeed(toHex(seed)).getSecretKey(),
    }];
  }
  ...
  async #createTransfer({ value, address, csFeeValue, csFeeAddress }) {
    ...
    if (this.crypto.type === 'coin') {
      ...
      const [first, ...rest] = this.#tokenObjects;
      if (rest.length > 0) {
        tx.mergeCoins(
    ...
    tx.setGasPayment(this.#coinObjects.map((coin) => {
      return {
        objectId: coin.coinObjectId,
        version: coin.version,
        digest: coin.digest,
  ...
  async loadTransactions({ cursor } = {}) {
    ...
    return {
      transactions,
      hasMore: data.transactionBlocks.pageInfo.hasPreviousPage,
      cursor: data.transactionBlocks.pageInfo.startCursor,
    };
  }

  #transformTx(tx) {
    try {
      const incoming = tx.sender.address !== this.#address;
      const status = tx.effects.status === 'SUCCESS' ? SUITransaction.STATUS_SUCCESS : SUITransaction.STATUS_FAILED;
      const timestamp = new Date(tx.effects.timestamp);
      const TOKEN_ADDRESS = this.crypto.type === 'coin' ? SUI_ADDRESS : this.crypto.address;
      if (incoming) {
        const transfer = tx.effects.balanceChanges.nodes.find((item) => {
          return item.coinType.repr === TOKEN_ADDRESS && item.owner.address === this.#address;
        });
        if (!transfer) return;
        ...
        if (this.crypto.type === 'coin') {
          ...
          const transfer = tx.effects.balanceChanges.nodes.find((item) => {
            return item.owner.address !== this.#address && item.csfee !== true;
          });
          if (!transfer) return;
          const minerFee = BigInt(tx.effects.gasEffects.gasSummary.computationCost) +
            BigInt(tx.effects.gasEffects.gasSummary.storageCost) -
            BigInt(tx.effects.gasEffects.gasSummary.storageRebate);
          const csFee = BigInt(tx.effects.balanceChanges.nodes.find((item) => {
            return item.csfee === true;
          })?.amount || 0);
          const isCoinTransaction = transfer.coinType.repr === SUI_ADDRESS;
          return new SUITransaction({
            id: tx.digest,
            to: transfer.owner.address,
            from: this.#address,
            amount: isCoinTransaction
              ? new Amount(parseInt(transfer.amount), this.crypto.decimals)
              : new Amount(0, this.crypto.decimals),
            incoming: false,
            fee: new Amount(minerFee + csFee, this.crypto.decimals),
            timestamp,
            confirmations: 1,
            minConfirmations: 1,
            status,
